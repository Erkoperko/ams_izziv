...
def get_loss_train(model, data_train, criterion):
    ...
    for batch, (images, masks) in enumerate(data_train):
        with torch.no_grad():
            ...
            #print(outputs[:,1,:,:].max(),outputs[:,1,:,:].min())
            loss = criterion(outputs, masks)
    ...
    for batch, (images_v, masks_v, original_msk) in enumerate(data_val):
        ...
        for index in range(images_v.size()[1]):
            with torch.no_grad():
                ...
                output_v = output_v.float()
                m = nn.Softmax(dim = 1)
                output_v = m(output_v)
                output_v[:,1,:,:] = (output_v[:,1,:,:]-torch.min(output_v[:,1,:,:]))/torch.max(output_v[:,1,:,:]-torch.min(output_v[:,1,:,:]))
                stacked_img = torch.cat((stacked_img, output_v[:,1,:,:]))
        if make_prediction:
    ...
    for batch, (images_t) in enumerate(data_test):
        ...
        for index in range(images_t.size()[1]):
            with torch.no_grad():
                ...
                output_t = output_t.float()
                m = nn.Softmax(dim = 1)
                output_t = m(output_t)
                output_t[:,1,:,:] = (output_t[:,1,:,:]-torch.min(output_t[:,1,:,:]))/torch.max(output_t[:,1,:,:]-torch.min(output_t[:,1,:,:]))
                stacked_img = torch.cat((stacked_img, output_t[:,1,:,:]))
        im_name = batch  # TODO: Change this to real image name so we know
        #m = nn.Softmax(dim = 0)
        #stacked_img = m(stacked_img)
        _ = save_prediction_image(stacked_img, im_name, epoch, save_folder_name)
        #print('saved image', saved_image.shape)
...
def save_prediction_image(stacked_img, im_name, epoch, save_folder_name="result_images", save_im=True):
    ...
    img_cont_np = ((img_cont)/div_arr)*255
    img_cont_np = img_cont_np.astype('uint8')
    #print(img_cont_np.shape, np.max(img_cont_np), np.min(img_cont_np))
    ...
    #sitk.WriteImage(sitk.GetImageFromArray(img_cont_np),desired_path+str(im_name)+'.nii.gz')
    return img_cont_np
